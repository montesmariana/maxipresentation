...
```{r xaringanExtra, eval=require("xaringanExtra"), echo=FALSE}
...
- `r simple_icons("d3-dot-js")` Visualization tool for exploration 
...
<!-- ### `r fontawesome("exclamation")` Models don't consistently disambiguate senses -->
...
<!-- Method as team of Fair Witnesses (in terms of Heinlein's *Stranger in a Strange Land*, 1961) -->
<!-- that could show us things we don't think of, going beyond what we expect to find. -->

<!-- We do not want to assume there is a golden standard (but what if we do? What if we want it?) -->

<!-- -> point out to neural networks!! -->
...
**word vectors** &rarr; numerical representations of words
...
.footnote[`r fontawesome("info-circle")` Actual values from GloWbE (Global Web-based English corpus).]
...
.center[
.bb.b--orange.ph3[.orange[(1)] Would you like to **study** *linguistics*?]

.bb.b--blue.ph3[.blue[(2)] They **study** this in *computational linguistics* too.]

.bb.b--green.ph3[.green[(3)] I eat *chocolate* while I **study**.]
...
.center[
```{r, echo=FALSE, eval=require("kableExtra")}
read_csv("tokensexample.csv", col_types = cols()) %>%
  kbl(escape = FALSE) %>% 
...
.gold[(1)] Would you like to **study** *linguistics*?

.blue[(2)] They **study** this in *computational linguistics* too.

.green[(3)] I eat *chocolate* while I **study**.
...
.small[(one occurrence, or all the occurrences)]
...
read_csv("vectorexample2.csv", col_types = cols()) %>%
  select(target, `language/n`, `word/n`, `english/j`, `speak/v`, everything()) %>% 
  mutate_if(is.numeric, round, 2) %>% 
  kbl(escape=FALSE) %>% 
  kable_styling(font_size = 9)
...
vexFoc <- read_csv("vectorexample3.csv", col_types = cols()) %>% 
...
- No set of parameters identifies "senses" reliably (~~best model~~).
